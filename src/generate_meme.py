...
async def generate_batch_memes(
    ...
    """
    Génère des mèmes par lots à partir d'un fichier JSON contenant des sujets
    
    Args:
        json_file_path: Chemin vers le fichier JSON contenant les sujets
        economy_mode: Utiliser le mode économie (GPT-3.5 au lieu de GPT-4)
        limit: Nombre maximum de mèmes à générer
        send_to_telegram: Envoyer les mèmes sur Telegram
    ...
    if not subjects:
        raise ValueError("Aucun sujet trouvé dans le fichier JSON.")
    ...
    # Générer les mèmes pour chaque sujet
    results = []
    ...
    if results:
        ...
        logger.info(f"📝 Rapport de génération par lots enregistré: {report_path}")
