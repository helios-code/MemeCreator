...
# Script pour faciliter l'utilisation de Docker avec le générateur de mèmes
...
show_help() {
    ...
    echo ""
    echo "Exemples:"
    echo "  ./run-docker.sh --build                     # Construire l'image Docker"
    echo "  ./run-docker.sh --generate \"Les politiciens\" # Générer un mème sur les politiciens"
    echo "  ./run-docker.sh --telegram \"Les médias\"      # Générer un mème et l'envoyer sur Telegram"
    echo "  ./run-docker.sh --json json.json 5          # Générer 5 mèmes à partir du fichier json.json"
    echo "  ./run-docker.sh --api                       # Lancer l'API web"
}
